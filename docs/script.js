...
var Demo = {
	init: function() {
		...
		// Example 17 - i18n
		var example17Data = {
			en: { invalid: 'Invalid', required: 'Required', email: 'Incorrect Email', message: 'Enter Message' },
			es: { invalid: 'Inválido', required: 'Necesario', email: 'Correo electrónico incorrecta', message: 'Introduzca Mensaje' },
			ru: { invalid: 'Неверно', required: 'Tребуется', email: 'Неправильное Email', message: 'Введите Cообщение' },
			it: { invalid: 'Non valido', required: 'Necessario', email: 'Errata Email', message: 'Inserisci Messaggio' },
			de: { invalid: 'Ungültig', required: 'Erforderlich', email: 'Falsche E-Mail Adresse', message: 'Geben Sie Nachricht' },
			fr: { invalid: 'Invalide', required: 'Requis', email: 'Adresse électronique incorrecte', message: 'Entrez Message' },
			jp: { invalid: '無効', required: '必須', email: 'メールアドレスが正しくありません', message: 'メッセージを入力してください' },
			he: { invalid: 'לא תקין', required: 'חובה', email: 'דוא״ל לא תקין', message: 'יש להזין הודעה' }
		}
		var example17 = $('#example-17').vForm($.extend({}, options, {
			onErrorMessage: function($field, tempMessage, key) {
				...
				message = example17Data[lang][key];
